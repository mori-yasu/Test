...
class camera
{
    public:
                ...
                double aspect)  //アスペクト比
        {
            ...
            //スクリーンの用意
            lower_left_corner = origin - half_width*u - half_height*v - w;  //スクリーンの開始位置。左下。(-w … スクリーンを距離1の所においてるってことになってるから)
            horizontal = 2.0*half_width*u;             //開始位置から水平にどんだけ移動するか。＝横幅
            vertical = 2.0*half_height*v;               //開始位置から垂直にどんだけ移動するか。=縦幅
        }
        ray get_ray(float s, float t)
        {
            //原点のカメラからスクリーン上の各ピクセルに飛ばした光線ベクトル
            return  ray(this->origin, 
                        this->lower_left_corner + s*this->horizontal + t*this->vertical - this->origin);
        }
