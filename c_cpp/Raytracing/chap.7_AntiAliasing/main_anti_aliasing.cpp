...
int main()
{
    ...
    const char *filepath ="D:\\workspace\\vsc\\c_cpp\\Raytracing\\one_weekend\\chap.7_AntiAliasing\\out_anti_aliasing_fine.ppm";
    ...
    else
    {
        ...
        for(int jpix = jpix_size - 1; jpix >= 0; jpix--)
        {
            for(int ipix = 0; ipix < ipix_size; ipix++)
            {
                ...
                {
                    srand(123L);    //初期化
                    float u = float(ipix + 2.0*genrand_real3() - 1.0) / float(ipix_size);      //水平方向の割合
                    float v = float(jpix + 2.0*genrand_real3() - 1.0) / float(jpix_size);      //垂直方向の割合
